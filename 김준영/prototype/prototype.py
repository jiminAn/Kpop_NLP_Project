...
data = pd.read_csv(path + 'LDA_text.tsv', sep='\t', encoding='utf-8')

ldamodel = gensim.models.LdaModel.load(path + '\\lda.model', mmap='r')
...
def recommend(topic, singer, title):
    ...
    for j in range(4):
        ...
        print("list{}: {} -- {}".format(j + 1, rcmd_data.loc[num].loc['제목'], rcmd_data.loc[num].loc['가수']))
